...
def load_data2(xt=[],yt=[],lb=[]):
    
    imgd = readxml(fxml1)
    imgd = readxml(fxml2,imgd)
    keys = np.array(list(imgd.keys()))
    ...
    for k in keys:
        ...
        if np.random.rand()<0.5:
            l = img_resize(flip(i3))
            l2 = img_resize(flip(v))
        else:
            l = img_resize(i3) 
            l2 = img_resize(v)
        ...
        if q<2000:
            ...
            if np.random.rand()<0.5:
                ...
                l = l + img_resize(flip(i32)) 
                l2 = l2 + img_resize(flip(v))
        
        for i in range(len(l)):
            xt.append(l[i])
            lb.append(k)
            yt.append(onehot(l2[i]))
        ...
        for i in range(nb):
            ...
            if i == nb-1:
                ...
                c2 = Conv2D(int(3*nf/4),
                          ...
                          kernel_regularizer=l2(1e-4),
                          name='CI'+str(i)+'_2')(xp)
                c04 = Conv2D(int(nf/16),
                          kernel_size=1,
                          strides=strides,
                          padding=padding,
                          kernel_initializer='he_normal',
                          kernel_regularizer=l2(1e-4),
                          name='CI'+str(i)+'_04')(xp)
                c4 = Conv2D(int(nf/8),
                          kernel_size=5,
                          strides=strides,
                          padding=padding,
                          kernel_initializer='he_normal',
                          kernel_regularizer=l2(1e-4),
                          name='CI'+str(i)+'_4')(c04)
                x3 = concatenate([c1,c2,c4],axis=3,name='CTI'+str(i))
            ...
            if i ==nb-1:
                x = Dropout(dropout,name='DP'+str(i+5))(x)
...
def weighted_binary_crossentropy(w,batch_size):
    ...
    def loss(y_true, y_pred):
        ...
        return K.pow(K.mean(weighted_b_ce),3/4) + K.pow(l,3/4)

    return loss 


def train_unet(model,lr = 0.0001,epochs = 8, batch_size = 8, save='temp',m=0.72):
    #tensorboard = TensorBoard(log_dir='C:\\Users\\AZEST-2019-07\\Desktop\\pyfiles\\logs\\tb1')
    ...
    checkpointer = ModelCheckpoint(filepath="/test/Ito/unet_weights2_"+save+".hdf5", 
    ...
    for i in range(epochs):
        ...
        if sc>m:
            ...
            epn = i
            print('saving.....',i)
            model.save_weights("/test/Ito/unet_weights2_"+save+".hdf5")
...
model,_ = resUnet(input_shape,dropout=0)
model.load_weights('/test/Ito/unet_weights2_temp.hdf5')
...
model = train_unet(model,lr = 0.0001,epochs = 6,batch_size = 8,save='temp',m=0.732)
